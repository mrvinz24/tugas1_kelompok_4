...
class Utils : 
    ...
    def postprocess_onnx(self, outs, frame, classes, 
            ...
            if frame is None:
                raise ValueError("Input frame is empty. Please check your image or camera source.")
            ...
            # Collect candidates above the confidence threshold before NMS.
            for i in range(rows):
            ...
            # OpenCV may return a list, tuple, or ndarray depending on the version.
            for index in np.array(result_boxes).reshape(-1):
                box = boxes[int(index)]
                x = box[0]
                y = box[1]
                w = box[2]
                ...
                label = '%s: %.1f%%' % (class_name, (scores[int(index)] * 100))
